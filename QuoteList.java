...
public class QuoteList {
    ...
    /* package */ static final int SearchKeywordVal = 3;
    ...
    */
    public QuoteList search(String searchString, int mode) {
        ...
        for (int i = 0; i < quoteArray.size(); i++) {
            ...
            if (mode == SearchAuthorVal && quote.getAuthor().toLowerCase().indexOf(searchString.toLowerCase()) != -1) { // Found a matching author, save it
                                                                                                                        // System.out.println ("Matched Author ");
                returnQuote.setQuote(quote);
            } else if (mode == SearchTextVal
                    && quote.getQuoteText().toLowerCase().indexOf(searchString.toLowerCase()) != -1) { // Found a matching quote, save it
                                                                                                                                        // System.out.println ("Matched Text ");
                returnQuote.setQuote(quote);
            } else if ((mode == SearchBothVal) && (quote.getAuthor().toLowerCase()
                    .indexOf(searchString.toLowerCase()) != -1
                    || quote.getQuoteText().toLowerCase().indexOf(searchString.toLowerCase()) != -1)) { // Found a matching author or quote, save it
                                                                                                                                                                                                                         // System.out.println ("Matched Both ");
                returnQuote.setQuote(quote);
            } else if (mode == SearchKeywordVal && quote.getKeyword() != null && quote.getKeyword().toLowerCase().indexOf (searchString.toLowerCase()) != -1) {
                returnQuote.setQuote (quote);
            }
        }
        return returnQuote;
